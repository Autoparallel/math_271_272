...
So, the set up to these problems is a bit different, but solving them is fundamentally the same idea. Let's see another example with slightly different boundary values.

\begin{ex}{Poisson's Equation: Dirichlet Boundary Data}{poisson_neumann}
\emph{Consider again a rod of length $L$ attached at endpoints $x=0$ and $x=L$.  Now, the angle of the rod is prescribed at the endpoints by $u(0)=0$ and $u(L)=0$. The rod is also being pushed down by a force more concentrated in the center and weaker on the edges.}\\

This gives us the boundary value problem
\[
\frac{d^2}{dx^2}(x)=\sin\left(\frac{x}{L}\pi\right)
\]
on the region $[0,L]$ with boundary data $u(0)=0$ and $u(L)=0$. We can integrate this equation twice to get
\[
u(x)=\frac{L^2}{\pi^2}\sin\left(\frac{x}{L}\pi\right)+C_1x+C_2.
\]
Now, we can use our boundary data to try and solve this problem. We require
\begin{align*}
    0=u(0)&=\frac{L^2}{\pi^2}\sin\left( \frac{0}{L}\pi\right)+C_1(0)+C_2\\
    0=u(L)&=\frac{L^2}{\pi^2}\sin\left(\frac{L}{L}\pi\right)+C_1(L)+C_2.
\end{align*}
The first equation gives us $C_2=0$ which we can plug into the second equation to get that $C_1=0$ as well.  Hence our particular solution to the boundary value problem is
\[
\boxed{u(x)=\frac{L^2}{\pi^2}\sin\left(\frac{x}{L}\pi\right).}
\]
\end{ex}

The above two equations are very common in the realm of differential equations.  Each gets its own name.  Let's write that here.
